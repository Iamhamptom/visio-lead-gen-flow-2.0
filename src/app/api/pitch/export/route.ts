...
<title>Visio Lead Gen Pitch — ${escapeHtml(dealerName)}</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .slide-badge {
    ...
    background: rgba(59,130,246,0.15); color: #34d399;
...
function buildMarkdown(slides: PitchSlide[], dealerName: string): string {
  const lines = [`# Visio Lead Gen Pitch — ${dealerName}\n`]
  ...
  lines.push(`---\n\n*Generated by Visio Lead Gen AI Pitch Builder*`)
  return lines.join('\n')
}
